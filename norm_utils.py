...
def average_sps(sps, mode='global'):
    """
    ...
    """
    if mode == 'global':
        # 모든 sps 데이터를 모아 전역 평균 계산
        sp_voiced_sum = None
        total_frames = 0
        for sp in sps:
            if sp_voiced_sum is None:
                sp_voiced_sum = np.sum(sp, axis=0)
            else:
                sp_voiced_sum += np.sum(sp, axis=0)
            total_frames += sp.shape[0]
        
        # 전역 평균 계산
        sp_avg = sp_voiced_sum / total_frames
        
        # 모든 sps를 평균으로 정규화
        for i in range(len(sps)):
            sps[i] = sps[i] / (sp_avg + 1e-6)

    elif mode == 'local':
        # 각 파일별로 평균값으로 정규화
        for i in range(len(sps)):
            sp = sps[i]
            sp_avg = np.mean(sp, axis=0)  # 각 주파수 대역별 평균 계산
            sps[i] = sp / (sp_avg + 1e-6)
