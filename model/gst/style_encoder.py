...
class VectorQuantizer(torch.nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        z_q = torch.matmul(min_encodings, self.vq_embedding.weight).view(z.shape)
        ...
        z_q_out = z_q.permute(0, 1, 2).contiguous()
        z_q_out = z_q_out.squeeze(-1)

        return z_q_out, vq_loss, min_encoding_indices

    def inference(self, style_embs):
        vq_style_embs = self.vq_embedding(style_embs)
        return vq_style_embs


class GST_VQ(torch.nn.Module):
    """Style encoder.
    ...
    def forward(self, speech: torch.Tensor) -> torch.Tensor:
        ...
        ref_embs = self.ref_enc(speech)
        style_embs = self.stl(ref_embs)
        vq_style_embs, vq_loss, min_encoding_indices = self.vq_layer(style_embs)

        return vq_style_embs, vq_loss, min_encoding_indices
