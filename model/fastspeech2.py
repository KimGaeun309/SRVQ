...
from .residual_vq_gaeun import ReferenceEncoderSRVQ3, SRVQ_PDE # 수정
    ...
    def forward(
        ...
        if not inference:
            ...
            if self.model_config["gst"]["use_gst"]:
                ...
                z_mel, z_pde, cls_loss = self.ref_enc(mels, p_targets=p_targets, d_targets=d_targets, e_targets=e_targets, emotions=emotions)
                if init_flag:
                    # kmeans_init !!!!
                    self.style_extractor.RVQ1.vq_layers[0].init_codebook_kmeans(z_mel)
                    self.style_extractor.RVQ2.vq_layers[0].init_codebook_kmeans(z_pde)
                    # self.style_extractor.RVQ3.vq_layers[0].init_codebook_kmeans(z_energy)

                style_ref_embs, vq_loss, min_encoding_indices, codebooks = self.style_extractor(z_mel, z_pde, cls_loss) 
                ...
                if init_flag:
                    ...
                    self.style_extractor.RVQ1.vq_layers[2].init_codebook_kmeans(z_mel - style_ref_embs[:, :128] - style_ref_embs[:, 128:256])
                    self.style_extractor.RVQ2.vq_layers[1].init_codebook_kmeans(z_pde - style_ref_embs[:, 384:512])
                    self.style_extractor.RVQ2.vq_layers[2].init_codebook_kmeans(z_pde - style_ref_embs[:, 384:512] - style_ref_embs[:, 512:640])
